...
function HorizontalScrollCarousel({ cardCount, cardsData }) {
    ...
    // Adjust the second value of useTransform to reduce horizontal movement length
    const x = useTransform(scrollYProgress, [0, 1], ["1%", "-60%"]); // Reduced from -98%
...
function Example({ numberOfCards, cardsData, startTitle }) {
    ...
    // Use scrollYProgress to trigger the fade-out effect
    const targetRef = useRef(null);
    const { scrollYProgress } = useScroll({ target: targetRef });
    // Adjust the second value of the range for a slower fade-out
    const opacity = useTransform(scrollYProgress, [0, 0.8], [1, 0]); // Slowed down fade-out
